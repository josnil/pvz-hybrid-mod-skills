#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""编译「向日葵女王僵尸 + 火焰向日葵舞者僵尸」Mod 的托管运行时程序集。

为什么要单独编译（纯数据做不到的部分）：
  · **3×3 友方（含自身）免疫减速与冻结**：内置的 `PeriodicAreaEventComponent` /
    `TowerDefenseExplode` 都走 `FillCharactersIntersectingRectListExcludingCamp`（排除**自己阵营**），
    「给友方加 buff」在数据侧没有通道 ⇒ 只能插件调
    `TowerDefenseExplode.CreateExplode(..., CHARACTER_CAMP.ALL, -1)` + `AddBuff(FireHit)`。
  · **每 0.5 秒 25 点灼烧**：内置植物《向日葵女王》是 `attackInterval = 3.0` + `Hurt.num = 40`，
    用户口径要 0.5s / 25 ⇒ 节拍与数值都得插件自己控。
  · **火球齐射的节拍**：`FireComponent` 的自发发射走「动画事件 fire」链，而本包的头是独立渲染的
    向日葵头、没有 `HeadFire` clip ⇒ 动画链路整条失效 ⇒ 1.5 秒的节拍必须由插件推进。
  （**6 颗本身走数据侧**：`fireProjectileList` 写 6 条、`firePosId` 0..5 各指一个 `FireMarker{i}`，
    一次 `Fire()` 同帧全打 —— 源码依据见生成器 docstring。）

入口类型 SunFlowerQueenRuntimeEntry 实现 IXWModRuntimeEntry（在 PlantsVsZombies.dll 里），
直接调用游戏的 FireComponent / TowerDefenseExplode / ResourceManager / Almanac 等公开成员，
所以必须引用 GodotSharp.dll + PlantsVsZombies.dll。
...
MOD_DIR = os.path.join(WORKSPACE, "SunFlowerQueen")
TARGET_DLL = os.path.join(MOD_DIR, "Runtime", "ModAssembly.dll")

CSPROJ = "SunFlowerQueenRuntime.csproj"
