#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""编译「超级机枪读报僵尸」Mod 的托管运行时程序集 SuperGatlingPaper/Runtime/ModAssembly.dll。

为什么要单独编译：
  需求 3（每 1.5 秒整点连发 7 颗）与需求 4（10% 概率、5 秒、±15° 随机散布 300 颗）
  **纯数据做不到** —— `FireComponentFireProjectileConfig` 只有 8 个字段
  （checkProjectileId / firePosId / speed / dir / offsetLine / fireNumSkip /
  fireEventNeed / projectileFlip），没有概率、没有时窗、没有逐发随机角；
  而 `FireComponent` 的自发发射走「有目标才发 → 动画事件 fire」这条链，
  节拍由动画长度决定，且读报僵尸的精灵根本没有开火动画（没有 Head 子精灵 / HeadFire clip）
  ⇒ 动画链路整条失效。所以豌豆必须由插件调公开的 `FireComponent.Fire()` 打出去。
  入口类型 SuperGatlingPaperRuntimeEntry 实现 IXWModRuntimeEntry（在 PlantsVsZombies.dll 里），
  直接调用游戏的 `FireComponent` / `ResourceManager` / `TowerDefenseCharacter` 等公开成员，
...
MOD_DIR = os.path.join(WORKSPACE, "SuperGatlingPaper")
TARGET_DLL = os.path.join(MOD_DIR, "Runtime", "ModAssembly.dll")

CSPROJ = "SuperGatlingPaperRuntime.csproj"
