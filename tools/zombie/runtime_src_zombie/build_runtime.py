#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""编译「暴走舞王伽刚特尔投石车僵尸」Mod 的托管运行时程序集 DiscoGargantuarPult/Runtime/ModAssembly.dll。

为什么要单独编译：
  需求里那条「投掷出来的僵尸改成暴走舞王伽刚特尔」**纯数据做不到**
  —— 内置小鬼投石车僵尸的 `ImpSpawn()` 把 `GetPacketConfig("ZombieImp")` 写死在代码里
  （Asset/Anime/Character/Zombie/Chapter5/Imppult/Scene/TowerDefenseZombieImppult.cs:135）。
  入口类型 DiscoGargantuarPultRuntimeEntry 实现 IXWModRuntimeEntry（在 PlantsVsZombies.dll 里），
  并直接调用游戏公开的 `CatapultComponent.OnFireEvent` / `TowerDefensePacketConfig.Create()` /
  `TowerDefenseGroundItemBase.characterNode` 等，所以必须引用 GodotSharp.dll + PlantsVsZombies.dll。
...
MOD_DIR = os.path.join(WORKSPACE, "DiscoGargantuarPult")
TARGET_DLL = os.path.join(MOD_DIR, "Runtime", "ModAssembly.dll")

CSPROJ = "DiscoGargantuarPultRuntime.csproj"
